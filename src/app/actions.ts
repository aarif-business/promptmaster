...
export async function seedChallengeImages(challengeId: string): Promise<{ seeded: number; skipped: number }> {
  ...
  const { data: challenge } = await admin.from('challenges').select('image_topic, difficulty').eq('id', challengeId).single()
  if (!challenge) throw new Error('Challenge not found')
  const count = IMAGES_PER_LEVEL[challenge.difficulty] ?? 10
  const { data: existing } = await admin.from('challenge_images').select('seed_index').eq('challenge_id', challengeId)
...
async function fetchImageFromTopic(topic: string, index: number): Promise<{ data: string; mimeType: string; resolvedUrl: string }> {
...
async function seedOneImage(challengeId: string, topic: string, seedIndex: number): Promise<{ id: string; imageUrl: string; referencePrompt: string }> {
  const admin = createAdminClient()
  const image = await fetchImageFromTopic(topic, seedIndex)
  const referencePrompt = buildReferencePrompt(topic)
  const { data: row, error } = await admin.from('challenge_images').insert({
    challenge_id: challengeId,
    image_url: image.resolvedUrl,
    image_data: image.data,
    image_mime_type: image.mimeType,
  ...
  }).select('id, image_data, image_mime_type, image_url, reference_prompt').single()
...
export async function getNextChallengeImage(challengeId: string): Promise<{ id: string; imageUrl: string; referencePrompt: string }> {
  ...
  const { data: seen } = await admin.from('user_image_assignments')
    .select('challenge_image_id').eq('user_id', user.id).eq('challenge_id', challengeId)
  ...
  await admin.from('user_image_assignments').upsert({
    user_id: user.id, challenge_id: challengeId, challenge_image_id: picked.id,
  }, { onConflict: 'user_id,challenge_image_id' })
...
export async function getDailyUsage(
  challengeId: string
): Promise<{ used: number; limit: number; allowed: boolean; resetsAt: string }> {
  const supabase = await createClient()
  const admin = createAdminClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return { used: 0, limit: 0, allowed: false, resetsAt: '' }

  const { data: challenge } = await admin.from('challenges').select('difficulty').eq('id', challengeId).single()
  const difficulty = challenge?.difficulty ?? 'beginner'
  const limit = DAILY_LIMITS[difficulty] ?? 1

  const startOfDay = new Date()
  startOfDay.setHours(0, 0, 0, 0)

  const { data: todaySubs } = await admin.from('submissions').select('id')
    .eq('user_id', user.id).eq('challenge_id', challengeId)
    .gte('submitted_at', startOfDay.toISOString())

  const used = (todaySubs ?? []).length
  const resetsAt = new Date(startOfDay.getTime() + 86400000).toISOString()

  return { used, limit, allowed: used < limit, resetsAt }
}
...
export async function checkLevelComplete(
  ...
  const { data: allInLevel } = await admin.from('challenges').select('id').eq('difficulty', challenge.difficulty)
  const ids = (allInLevel ?? []).map(c => c.id)

  const { data: passed } = await admin.from('submissions').select('challenge_id')
    .eq('user_id', user.id).eq('passed', true).in('challenge_id', ids)

  const passedIds = new Set((passed ?? []).map(s => s.challenge_id))
  return { levelComplete: ids.every(id => passedIds.has(id)), difficulty: challenge.difficulty }
}
...
export async function evaluatePrompt(
  ...
  // Enforce daily limit
  const { data: challengeRow } = await admin.from('challenges').select('difficulty').eq('id', challengeId).single()
  const difficulty = challengeRow?.difficulty ?? 'beginner'
  const limit = DAILY_LIMITS[difficulty] ?? 1
  const startOfDay = new Date()
  startOfDay.setHours(0, 0, 0, 0)
  const { data: todaySubs } = await admin.from('submissions').select('id')
    .eq('user_id', user.id).eq('challenge_id', challengeId)
    .gte('submitted_at', startOfDay.toISOString())
  if ((todaySubs ?? []).length >= limit) {
    return {
      score: 0,
      feedback: `You've reached today's limit of ${limit} attempt${limit > 1 ? 's' : ''} for this challenge. Come back tomorrow!`,
      passed: false,
      error: 'daily_limit',
    }
  }

  const { data: imgRow } = await admin.from('challenge_images')
    .select('reference_prompt').eq('id', challengeImageId).single()
